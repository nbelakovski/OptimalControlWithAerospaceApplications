...
sol_mass = solve_bvp(ascent_odes_tf,ascent_bcs_tf,tau, solinit_mass, np.array([tf-delta_tf]), verbose=2)
# Evaluate the solution at all times in the nondimensional time vector tau
...
sol_mass_drag = solve_bvp(ascent_odes_tf,ascent_bcs_tf, tau, solinit_mass_drag, np.array([tf_mass]), verbose=2)
# Evaluate the solution at all times in the nondimensional time vector tau
...
plt.xlabel('Downrange Position, x [km]')
plt.ylabel('Altitude, y [km]')
plt.xlim(x_sol_mass_drag[0], x_sol_mass_drag[-1])
plt.ylim(0, 200)
